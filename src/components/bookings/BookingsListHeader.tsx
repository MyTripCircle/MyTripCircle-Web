...
/**
 * En-tête de la liste des réservations pour les paliers sans grand titre de page
 * (mobile et tablette) : compteur, titre et action d'ajout.
 */
const BookingsListHeader: React.FC<Props> = ({ count, onAdd, addDisabled, addStyle, flush = false }) => {
  ...
  return (
    <View style={[styles.header, flush && styles.headerFlush]}>
      <View>
        <Text style={[styles.eyebrow, { color: colors.textLight }]}>
          {t("bookings.count", { count })}
        </Text>
        <Text style={[styles.title, { color: colors.text }]}>{t("bookings.header")}</Text>
      </View>
      <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: F.sans700, fontSize: 28 },
  addButton: {
    width: 44,
    height: 44,
    borderRadius: 22,
